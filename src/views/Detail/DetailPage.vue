            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="count <= 1 ? 1 : count--"
                  >-</a
                >
              </div>
              <div class="add">
                <a @click="sendShopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("detailInfo/getGoodsData", this.$route.params.pdid);
  },
  computed: {
    ...mapGetters("detailInfo", ["categoryView", "skuInfo", "spuSaleAttrList"]),
  },
  methods: {
    chagneActive(AttrValue, AttrValueList) {
